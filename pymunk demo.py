...
def main():
    running = True
    Pmanager = PhysicsManager(screen,FLAG_DRAW_SURFACES=False,FLAG_DRAW_SHAPES=True, FLAG_DRAW_BODIES=True)
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if right:
                    #sample
                    print(f'body_pos{camera1.current}, camera {camera1.dxy}')
                    print(f'cog {Object_list[selector_i]}')
        ...
        clock.tick(fps)
        space.step(1/fps)
        camera1.update(Object_list[selector_i])
